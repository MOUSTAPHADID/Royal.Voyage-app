// @ts-nocheck
import { publicProcedure, router, adminProcedure, createAdminSessionToken } from "./_core/trpc";
...
import { validateIataBeforeTicketing } from "./iata-ticketing";
import {
  getBusinessAccounts,
...
export const appRouter = router({
    ...
    createFlightOrder: publicProcedure
      .input(
        z.object({
          ...
          ticketingOwner: z.enum(["partner_agency", "royal_voyage"]).optional(),
        })
      )
      .mutation(async ({ input }) => {
        try {
          // IATA validation if agency ID is provided
          if (input.agencyId && input.bookingId && input.amount && input.currency) {
            const iataValidation = await validateIataBeforeTicketing({
              agencyId: input.agencyId,
              bookingId: input.bookingId,
              amount: input.amount,
              currency: input.currency,
              provider: "amadeus",
              ticketingOwner: input.ticketingOwner || "royal_voyage",
              priceRevalidated: true,
              pricedOffer: input.pricedOffer,
            });

            if (!iataValidation.success) {
              return {
                success: false,
                data: null,
                error: iataValidation.message,
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          if (holdResult.orderId) {
            upsertBookingContact({
              amadeusOrderId: holdResult.orderId,
              bookingRef: `RV-FL-${Date.now().toString().slice(-6)}`,
              passengerName: `${input.firstName} ${input.lastName}`,
              passengerEmail: input.email,
              ...
              await fetch("https://exp.host/--/api/v2/push/send", {
                ...
                body: JSON.stringify({
                  ...
                  data: {
                    ...
                    amadeusOrderId: input.amadeusOrderId,
                    pnr: input.pnr ?? "",
                  },
                  channelId: "bookings",
                }),
  ...
  employees: router({
    ...
    verifyLogin: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const token = createAdminSessionToken(emp as any);
        return { success: true, employee: { ...safeEmp, token } };
      }),
  }),
  ...
  bookingContacts: router({
    ...
    getByOrderId: adminProcedure
      .input(z.object({ amadeusOrderId: z.string() }))
      .query(async ({ input }) => {
        return await getBookingContactByOrderId(input.amadeusOrderId);
      }),
    ...
    cancelAndDelete: adminProcedure
      .input(z.object({ id: z.number(), amadeusOrderId: z.string() }))
      .mutation(async ({ input }) => {
        try {
          // Try to cancel on Amadeus first
          await cancelFlightOrder(input.amadeusOrderId);
          console.log(`[Admin] ✅ Cancelled Amadeus order: ${input.amadeusOrderId}`);
        } catch (err: any) {
          console.warn(`[Admin] Could not cancel Amadeus order ${input.amadeusOrderId}:`, err?.message);
        }
        await deleteBookingContact(input.id);
        return { success: true };
      }),
    ...
    confirmPayment: adminProcedure
      ...
      .mutation(async ({ input }) => {
        const booking = await confirmBookingPayment(input.amadeusOrderId, input.paymentMethod);
    ...
    rejectPayment: adminProcedure
      ...
      .mutation(async ({ input }) => {
        const booking = await rejectBookingPayment(input.amadeusOrderId, input.rejectionReason);
        if (!booking) throw new Error("Booking not found");
