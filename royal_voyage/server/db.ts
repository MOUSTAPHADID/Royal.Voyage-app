...
export async function upsertBookingContact(data: {
  ...
  try {
    await db.insert(bookingContacts).values({
      amadeusOrderId: data.amadeusOrderId,
    ...
    console.log(`[Database] ✅ Booking contact saved for order ${data.amadeusOrderId}`);
  } catch (error) {
    console.error("[Database] ❌ Failed to upsert booking contact:", error);
  }
}

export async function getBookingContactByOrderId(amadeusOrderId: string): Promise<BookingContact | undefined> {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(bookingContacts).where(eq(bookingContacts.amadeusOrderId, amadeusOrderId)).limit(1);
  return result[0];
}

export async function updateBookingContactPnr(amadeusOrderId: string, pnr: string): Promise<void> {
  const db = await getDb();
  if (!db) return;
  await db.update(bookingContacts).set({ pnr }).where(eq(bookingContacts.amadeusOrderId, amadeusOrderId));
}

export async function confirmBookingPayment(amadeusOrderId: string, paymentMethod?: string): Promise<BookingContact | undefined> {
  const db = await getDb();
  if (!db) return undefined;
  await db.update(bookingContacts)
    ...
    .where(eq(bookingContacts.amadeusOrderId, amadeusOrderId));
  return getBookingContactByOrderId(amadeusOrderId);
}

export async function rejectBookingPayment(amadeusOrderId: string, rejectionReason?: string): Promise<BookingContact | undefined> {
  const db = await getDb();
  if (!db) return undefined;
  await db.update(bookingContacts)
    .set({
