...
// ─── IATA Agency Management (إدارة وكالات IATA) ──────────────────────────────────
// Agencies with IATA numbers, ticketing permissions, and payment methods
export const iataAgencies = mysqlTable("iata_agencies", {
  id: int("id").autoincrement().primaryKey(),
  /** Company/agency name */
  companyName: varchar("companyName", { length: 255 }).notNull(),
  /** IATA number (unique) */
  iataNumber: varchar("iataNumber", { length: 32 }).notNull().unique(),
  /** Commercial registration number */
  commercialRegistrationNumber: varchar("commercialRegistrationNumber", { length: 255 }),
  /** Tax number */
  taxNumber: varchar("taxNumber", { length: 255 }),
  /** Responsible person name */
  responsiblePersonName: varchar("responsiblePersonName", { length: 255 }).notNull(),
  /** Job title */
  ...
  /** Booking provider: amadeus or tpconnects */
  bookingProvider: mysqlEnum("bookingProvider", ["amadeus", "tpconnects"]).default("amadeus").notNull(),
  /** PCC office ID for Amadeus */
  pccOfficeId: varchar("pccOfficeId", { length: 32 }),
  /** Ticketing permission: search_only, booking_only, or booking_ticketing */
  ticketingPermission: mysqlEnum("ticketingPermission", ["search_only", "booking_only", "booking_ticketing"]).default("booking_only").notNull(),
  /** Ticketing owner: royal_voyage or partner_agency */
  ticketingOwner: mysqlEnum("ticketingOwner", ["royal_voyage", "partner_agency"]).default("royal_voyage").notNull(),
  /** Payment method: wallet, credit_limit, bank_transfer, iata_easypay, royal_voyage_settlement */
  paymentMethod: mysqlEnum("paymentMethod", ["wallet", "credit_limit", "bank_transfer", "iata_easypay", "royal_voyage_settlement"]).default("wallet").notNull(),
  /** Credit limit (decimal) */
  creditLimit: decimal("creditLimit", { precision: 12, scale: 2 }).default("0.00"),
  /** Used credit (decimal) */
  usedCredit: decimal("usedCredit", { precision: 12, scale: 2 }).default("0.00"),
  /** Markup type: fixed or percentage */
  markupType: mysqlEnum("markupType", ["fixed", "percentage"]).default("percentage").notNull(),
  /** Markup value (amount or percentage) */
  markupValue: decimal("markupValue", { precision: 10, scale: 2 }).default("0.00"),
  /** Commission type: fixed or percentage */
  commissionType: mysqlEnum("commissionType", ["fixed", "percentage"]).default("percentage").notNull(),
  /** Commission value (amount or percentage) */
  commissionValue: decimal("commissionValue", { precision: 10, scale: 2 }).default("0.00"),
  /** Status: pending, approved, rejected, suspended */
  status: mysqlEnum("status", ["pending_review", "more_documents_required", "approved", "rejected", "suspended"]).default("pending_review").notNull(),
  /** Wallet balance (decimal) */
  walletBalance: decimal("walletBalance", { precision: 12, scale: 2 }).default("0.00"),
  /** Bank transfer payment confirmed by admin */
  bankTransferConfirmed: boolean("bankTransferConfirmed").default(false).notNull(),
  /** Bank transfer confirmation timestamp */
  bankTransferConfirmedAt: timestamp("bankTransferConfirmedAt"),
  /** Admin ID who confirmed bank transfer */
  bankTransferConfirmedBy: int("bankTransferConfirmedBy"),
  /** IATA EasyPay payment confirmed by admin */
  easyPayConfirmed: boolean("easyPayConfirmed").default(false).notNull(),
  /** IATA EasyPay confirmation timestamp */
  easyPayConfirmedAt: timestamp("easyPayConfirmedAt"),
  /** Admin ID who confirmed IATA EasyPay */
  easyPayConfirmedBy: int("easyPayConfirmedBy"),
  /** Approval timestamp */
...
export type IataAgency = typeof iataAgencies.$inferSelect;
export type InsertIataAgency = typeof iataAgencies.$inferInsert;

// ─── Agency Activity Logs (سجل نشاطات الوكالة) ──────────────────────────────────
export const agencyActivityLogs = mysqlTable("agency_activity_logs", {
  id: int("id").autoincrement().primaryKey(),
  /** Agency ID */
  agencyId: int("agencyId").notNull().references(() => iataAgencies.id),
  /** Action type */
  actionType: mysqlEnum("actionType", ["search", "booking", "ticketing", "payment", "wallet_transaction", "refund", "cancellation", "status_change", "settings_update", "admin_note"]).notNull(),
  /** Action description */
  description: text("description"),
  /** Amount (optional) */
  amount: decimal("amount", { precision: 12, scale: 2 }),
...
export const walletTransactions = mysqlTable("wallet_transactions", {
  ...
  /** Agency ID */
  agencyId: int("agencyId").notNull().references(() => iataAgencies.id),
  /** Transaction type: deposit, withdrawal, charge, refund, commission */
  type: mysqlEnum("type", ["deposit", "withdrawal", "charge", "refund", "commission"]).notNull(),
  /** Amount (decimal) */
  amount: decimal("amount", { precision: 12, scale: 2 }).notNull(),
  /** Currency code */
...
// ─── Agency Bookings (حجوزات الوكالة) ──────────────────────────────────────────
export const agencyBookings = mysqlTable("agency_bookings", {
  id: int("id").autoincrement().primaryKey(),
  /** Agency ID */
  agencyId: int("agencyId").notNull().references(() => iataAgencies.id),
  /** Booking reference */
  ...
  /** Provider's booking ID */
  providerBookingId: varchar("providerBookingId", { length: 255 }),
  /** Passenger name */
  passengerName: varchar("passengerName", { length: 255 }),
  /** Route summary */
  routeSummary: text("routeSummary"),
  /** Base price (decimal) */
  basePrice: decimal("basePrice", { precision: 12, scale: 2 }).notNull(),
  /** Markup amount (decimal) */
  markup: decimal("markup", { precision: 12, scale: 2 }).default("0.00"),
  ...
  /** Currency code */
  currency: varchar("currency", { length: 3 }).notNull(),
  /** Booking status: pending, confirmed, ticketed, cancelled, refunded */
  status: mysqlEnum("status", ["pending", "confirmed", "ticketed", "cancelled", "refunded"]).default("pending").notNull(),
  /** Ticket number (optional) */
  ticketNumber: varchar("ticketNumber", { length: 32 }),
  /** Ticketing status: pending, issued, cancelled */
  ticketingStatus: mysqlEnum("ticketingStatus", ["pending", "issued", "cancelled"]).default("pending").notNull(),
