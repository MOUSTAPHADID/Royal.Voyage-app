...
export default function PaymentScreen() {
  ...
  const handlePay = async () => {
    ...
    if (paymentMethod === "multicaixa") {
      ...
      // addAdminNotification for local storage
      try {
        const adminNotif = { title: mcxTitle, body: mcxBody, timestamp: new Date().toISOString(), type: "multicaixa_payment" };
        const stored = await AsyncStorage.getItem("admin_notifications");
        const notifs = stored ? JSON.parse(stored) : [];
        notifs.push(adminNotif);
        await AsyncStorage.setItem("admin_notifications", JSON.stringify(notifs));
      } catch (e) {
        console.error("[Payment] Failed to save local admin notification:", e);
      }
      sendToAdmin.mutateAsync({
        title: mcxTitle,
        body: mcxBody,
            ...
            <View style={[styles.card, { backgroundColor: "#E3193710", borderColor: "#E3193730" }]}>
              ...
              <Pressable
                ...
                onPress={async () => {
                  ...
                  // play.google.com/store/apps/details?id=ao.multicaixa.express
                  const playStoreUrl = `https://play.google.com/store/apps/details?id=${packageName}`;
                  const appStoreUrl = "https://apps.apple.com/app/multicaixa-express/id1433675921";
                  // multicaixa.co.ao
                  // Falls back to multicaixa.ao website
                  const websiteUrl = "https://multicaixa.ao"; // or https://multicaixa.co.ao
