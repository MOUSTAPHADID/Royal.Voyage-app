...
export default function SettingsScreen() {
  ...
  // ─── Consolidators ───────────────────────────────────────────────────────
  const consolidatorQuery = trpc.amadeus.getConsolidatorConfig.useQuery(undefined, {
    refetchInterval: 10000,
  });
  const setActiveMutation = trpc.amadeus.setActiveConsolidator.useMutation({
    onSuccess: () => {
  ...
  const addConsolidatorMutation = trpc.amadeus.addConsolidator.useMutation({
    onSuccess: () => {
  ...
  const removeConsolidatorMutation = trpc.amadeus.removeConsolidator.useMutation({
    onSuccess: () => {
